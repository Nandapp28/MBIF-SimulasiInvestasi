...
public class SellingPhaseManagerMultiplayer : MonoBehaviourPunCallbacks
{
    ...
    void Awake()
    {
        if (Instance != null) Destroy(gameObject);
            ...
            initialIpoPositions.Clear();
        foreach (var mapping in ipoIndicatorMappings)
        {
            if (mapping.indicatorObject != null)
            {
                initialIpoPositions[mapping.color] = mapping.indicatorObject.transform.position;
            }
        }
    }
    ...
    private void UpdateAllIpoVisuals()
    {
        ...
        foreach (var mapping in ipoIndicatorMappings)
        {
            ...
            if (roomProps.ContainsKey(ipoKey))
            {
                ...
                // --- TAMBAHAN LOGIKA KHUSUS UNTUK TAMBANG ---
                if (mapping.color == "Tambang")
                {
                    // Clamp index antara -2 dan 2 untuk keamanan
                    ipoIndex = Mathf.Clamp(ipoIndex, -2, 2);
                    // Ubah dari index -2 -> 2 menjadi index array 0 -> 4
                    positionIndex = ipoIndex + 2; 
                }
                else // Logika untuk sektor lain tetap sama
                {
                    // Clamp index antara -3 dan 3
                    ipoIndex = Mathf.Clamp(ipoIndex, -3, 3);
                    // Ubah dari index -3 -> 3 menjadi index array 0 -> 6
                    positionIndex = ipoIndex + 3;
                }
                // ---------------------------------------------

                if (mapping.indicatorObject != null && positionIndex >= 0 && positionIndex < mapping.positionSlots.Count)
                {
    ...
    public int GetFullCardPrice(string color)
    {
        ...
        int salesBonus = roomProps.ContainsKey(IPO_BONUS_PREFIX + color) ? (int)roomProps[IPO_BONUS_PREFIX + color] : 0;
        int basePrice = GetCurrentColorValue(color, ipoIndex);
        return basePrice + salesBonus;
    }
    ...
    private int GetCurrentColorValue(string color, int ipoIndex)
    {
        if (ipoPriceMap.ContainsKey(color))
        {
            if (color == "Tambang")
                ...
                ipoIndex = Mathf.Clamp(ipoIndex, -3, 3);
            int mapIndex = ipoIndex + 3;
            return ipoPriceMap[color][mapIndex];
        }
        return 0;
    }
